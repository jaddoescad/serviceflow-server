-- Fix get_deal_proposal_data RPC to use correct product_templates columns
-- product_templates has: id, company_id, created_by_user_id, name, description, type, created_at, updated_at
-- No default_price or position columns
...
BEGIN
  ...
  IF p_quote_id IS NOT NULL THEN
    SELECT json_build_object(
      ...
      'acceptance_signer_name', q.acceptance_signer_name,
  ...
  -- Get product templates with correct columns (no default_price or position)
  SELECT COALESCE(json_agg(
    json_build_object(
      'id', pt.id,
