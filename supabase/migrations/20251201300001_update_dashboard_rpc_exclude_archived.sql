-- Update get_dashboard_data RPC to exclude archived deals from Kanban
-- and include archived_at in the response for other views
...
BEGIN
  -- Get pipeline stages dynamically from the database
  -- This uses the get_pipeline_stage_keys function which handles initialization
  v_pipeline_stages := get_pipeline_stage_keys(p_company_id, p_pipeline_id);

  -- If no stages found (shouldn't happen after initialization), use empty array
  IF v_pipeline_stages IS NULL OR array_length(v_pipeline_stages, 1) IS NULL THEN
    v_pipeline_stages := ARRAY[]::TEXT[];
  END IF;

  -- Get deals with contact, service_address, and latest_appointment
  -- Exclude archived deals (archived_at IS NOT NULL)
