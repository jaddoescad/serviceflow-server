-- Fix RPC to remove non-existent deal_attachments table
...
BEGIN
  ...
  -- Get proposal attachments (this is the only attachment table)
  SELECT COALESCE(json_agg(
    json_build_object(
      'id', pa.id,
      'deal_id', pa.deal_id,
  ...
  -- Return combined result (removed separate attachments field)
