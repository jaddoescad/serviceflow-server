-- Fix RPCs to use correct proposal_attachments column names:
-- original_filename (not file_name)
-- byte_size (not size_bytes)
-- uploaded_at (not created_at)
-- no position column exists

-- Fix get_deal_detail RPC
...
-- Fix get_deal_proposal_data RPC
...
BEGIN
  ...
  SELECT COALESCE(json_agg(
    json_build_object(
      ...
      'type', pt.type,
      'default_price', pt.default_price,
      'position', pt.position,
      'created_at', pt.created_at
    ) ORDER BY pt.position, pt.name
