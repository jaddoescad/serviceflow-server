-- Add archived_at to get_deal_proposal_data RPC response
...
BEGIN
  -- Get deal with contact and service_address (now includes archived_at)
  ...
  -- Get proposal template with correct field names
  SELECT json_build_object(
    'id', t.id,
  ...
  -- Get work order template with correct field names
  SELECT json_build_object(
    'id', t.id,
    'company_id', t.company_id,
    'template_key', t.template_key,
  ...
  -- Get change order template with correct field names
  SELECT json_build_object(
    'id', t.id,
    'company_id', t.company_id,
    'template_key', t.template_key,
