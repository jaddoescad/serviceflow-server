-- RPC to get public quote share data in one call
-- Used for the public quote/proposal view page
-- Consolidates: quote + change orders + invoice into one database call
...
BEGIN
  ...
  SELECT
    ...
    json_build_object(
      ...
      'website', co.website,
      'owner_first_name', co.owner_first_name,
      'owner_last_name', co.owner_last_name
    ),
  ...
  WHERE q.public_share_id = p_share_id;
  ...
  SELECT COALESCE(json_agg(
    json_build_object(
      ...
      'items', COALESCE(
        ...
        ) FROM change_order_items coi WHERE coi.change_order_id = co.id),
        '[]'::json
      )
    ) ORDER BY co.created_at
  ), '[]'::json) INTO v_change_orders
  FROM change_orders co
  WHERE co.quote_id = v_quote_id;
