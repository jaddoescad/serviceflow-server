-- Add drip sequence metadata to get_deal_detail RPC
-- This eliminates the need for a separate API call to fetch drip sequences
-- Only includes minimal metadata needed for UI (stage_id, is_enabled, step_count)
...
DECLARE
  ...
  v_drip_sequences_meta JSON;
BEGIN
  -- Get deal with contact, service_address, and crew
  SELECT json_build_object(
    ...
    'crew', CASE WHEN cr.id IS NOT NULL THEN json_build_object(
      'id', cr.id,
      'name', cr.name
    ) ELSE NULL END,
    'latest_appointment', (
  ...
  LEFT JOIN crews cr ON d.crew_id = cr.id
  ...
  -- Get pipeline_id for drip sequences (default to 'sales' for now)
  v_pipeline_id := 'sales';
  ...
  -- Get drip sequence metadata (lightweight - only stage_id, is_enabled, step_count)
  SELECT COALESCE(json_agg(
    json_build_object(
      'stage_id', ds.stage_id,
      'is_enabled', ds.is_enabled,
      'step_count', (SELECT COUNT(*) FROM drip_steps WHERE sequence_id = ds.id)
    )
  ), '[]'::json) INTO v_drip_sequences_meta
  FROM drip_sequences ds
  WHERE ds.company_id = v_company_id
    AND ds.pipeline_id = v_pipeline_id;
  ...
  RETURN json_build_object(
    ...
    'dripSequencesMeta', v_drip_sequences_meta
  );
END;
$$;
